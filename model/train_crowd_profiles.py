...
EXCLUDED_LOCATION_IDS = {"28", "65", "78"}
...
def main() -> int:
    ...
    model_clean = clean[~clean["location_id"].isin(EXCLUDED_LOCATION_IDS)].copy()
    metadata_counts["excluded_location_row_count"] = int(len(clean) - len(model_clean))
    if model_clean.empty:
        raise ValueError("No model rows remain after excluding removed sensors.")

    profiles = build_profiles(model_clean, actual_window)
    validation = build_validation_metrics(model_clean)
...
def build_metadata(
    ...
    metadata = {
        ...
        "excluded_location_ids": sorted(EXCLUDED_LOCATION_IDS, key=int),
        "excluded_location_row_count": counts.get("excluded_location_row_count", 0),
    }
    return metadata
